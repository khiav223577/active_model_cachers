...
class ActiveModelCachersTest < Minitest::Test
  ...
  def test_has_one_cache
    ...
    assert_queries(1){ assert_equal 10, User.cacher(profile.id).profile.point }
    assert_cache('cacher_key_of_Profile_1' => profile)

    assert_queries(0){ assert_equal 10, User.cacher(profile.id).profile.point }
    assert_cache('cacher_key_of_Profile_1' => profile)
  end
  ...
  def test_has_one_cache_when_update
    ...
    assert_queries(1){ assert_equal 12, User.cacher(profile.id).profile.point }
    assert_cache('cacher_key_of_Profile_1' => profile)
  ensure 
  ...
  def test_has_one_cache_when_destroy
    ...
    assert_queries(1){ assert_equal 13, User.cacher(profile.id).profile.point }
    assert_cache("cacher_key_of_Profile_#{profile.id}" => profile)

    profile.destroy
    assert_cache({})

    assert_queries(1){ assert_nil User.cacher(profile.id).profile }
    assert_cache({})
  ensure
    profile.destroy
  end
  ...
  def test_has_one_cache_when_destroyed_by_dependent_delete
    ...
    assert_queries(1){ assert_equal 17, User.cacher(profile.id).profile.point }
    assert_cache("cacher_key_of_Profile_#{profile.id}" => profile)
