...
class CacheAtBelongsToTest < BaseTest
  def test_basic_usage
    ...
    assert_queries(1){ assert_equal 'zh-tw', User.cacher_at(user.id).language.name }
    assert_queries(0){ assert_equal 'zh-tw', User.cacher_at(user.id).language.name }
    assert_cache('active_model_cachers_User_at_language1' => language)
  end
  ...
  def test_create
    ...
    assert_cache("active_model_cachers_User_at_language#{user.id}" => ActiveModelCachers::NilObject)
    ...
    assert_cache("active_model_cachers_User_at_language#{user.id}" => language)
  ensure
